...
is_md5(void * expected, void * got, const char * str)
{
    ...
    diag("Expected MD5:");
    SXED1(expected, sizeof(SOPHOS_MD5));
    diag("Got      MD5:");
    SXED1(got,      sizeof(SOPHOS_MD5));
    fail("%s", str);
}
...
main(void)
{
     ...
     */
    MD5_Init(  &md5);
    MD5_Update(&md5, (const unsigned char *)TESTB, strlen(TESTB));
    MD5_Final( &Message_Digest, &md5);
    ...
    for(i = 1; i <= 1000000; i++) {
        MD5_Update(&md5, (const unsigned char *)TESTC, 1);
    }
     ...
     */
    MD5_Init(  &md5);
    MD5_Update(&md5, (const unsigned char *)TESTQ, strlen(TESTQ));
    MD5_Final(&Message_Digest, &md5);
