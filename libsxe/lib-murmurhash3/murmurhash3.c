...
static FORCE_INLINE void MurmurHash3_x86_128 ( const void * key, const int len,
                           uint32_t seed, void * out )
{
  ...
  case  1: k1 ^= tail[ 0] << 0;
           k1 *= c1; k1  = ROTL32(k1,15); k1 *= c2; h1 ^= k1; /* FALLTHRU */
  };
...
static FORCE_INLINE void MurmurHash3_x64_128 ( const void * key, const int len,
...
void MurmurHash3_xnn_128 ( const void * key, const int len,
                           const uint32_t seed, void * out )
{
#if SXE_OS_BITS == 64
...
uint32_t
murmur3_32(const void *key, uint32_t len, uint32_t seed)
{
    ...
    switch (len & 3) {
    case 3:
        k1 ^= tail[2] << 16; /* FALLTHRU */
    case 2:
        k1 ^= tail[1] << 8; /* FALLTHRU */
    case 1:
        k1 ^= tail[0];
